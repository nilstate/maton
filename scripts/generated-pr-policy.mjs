export const GENERATED_PR_POLICY_MARKER = "<!-- automaton:generated-pr-policy";
...
export function parseGeneratedPrPolicy(body) {
  const match = String(body ?? "").match(/<!-- automaton:generated-pr-policy\s+lane=([^\s]+)\s+merge=([^\s]+)\s+draft_only=(true|false)\s*-->/);
  if (!match) {
    return null;
  }
  return {
