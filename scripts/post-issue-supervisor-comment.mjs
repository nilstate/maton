...
const currentMarker = "<!-- automaton:runx-issue-supervisor -->";
const legacyMarker = "<!-- automaton:runx-support-triage -->";
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const existing = (issue.comments ?? []).find(
    (comment) => typeof comment.body === "string"
      && (comment.body.includes(currentMarker) || comment.body.includes(legacyMarker)),
  );
